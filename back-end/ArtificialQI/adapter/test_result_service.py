...
class TestResultService(TestResultUseCase):
    ...
    def get_result_page(self, test_id: UUID, page_n: int, q: str="") -> Page[TestResult]:
        ...
        test_result_set: Optional[set[TestResult]] = self.test_result_repo.get_results(test_id, offset, offset + Page.ELEMENT_PER_PAGE, q)

        if test_result_set is None:
            raise PageNonExistentException(page_n)
