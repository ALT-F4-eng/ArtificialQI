...
def update_test(id: str, test_service: TestService = Provide[AppContainer.test_service],
    ...
    # Ora chiama correttamente to_dto
...
@inject
def get_all_tests(test_service: TestService = Provide[AppContainer.test_service]):
    query: Optional[str] = request.args.get("q", "")

    results: list[Test] = test_service.get_all_tests(query)
