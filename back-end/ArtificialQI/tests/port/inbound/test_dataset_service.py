...
from common.exceptions import PersistenceException, DatasetNonExistentException, InvalidDatasetOperationException
...
class _TestDatasetService:
    ...
    def test_copy_tmp_dataset(self, mocked_dataset_service):
        ...
        mocked_dataset_service[
            "dataset_repo"
        ].get_dataset_by_id.return_value.is_saved.return_value = False

        with pytest.raises(InvalidDatasetOperationException):
            mocked_dataset_service["dataset_service"].copy_dataset(uuid4())

    @patch("adapter.dataset_service.DatasetFactory.saved")
    def test_copy_saved_empty_dataset_with_persistence_error(
        self, mock_datasetfactory_saved, mocked_dataset_service
    ):
    ...
    @patch("adapter.dataset_service.DatasetFactory.saved")
    def test_copy_saved_non__empty_dataset_with_persistence_error(
        self, mock_datasetfactory_saved, mocked_dataset_service
    ):
    ...
    @patch("adapter.dataset_service.DatasetFactory.saved")
    def test_copy_saved_empty_dataset(
        self, mock_datasetfactory_saved, mocked_dataset_service
    ):

        dataset_copy_mock = MagicMock()
        mock_datasetfactory_saved.return_value.dim = 0
        mocked_dataset_service["dataset_repo"].create_dataset.return_value = (
            dataset_copy_mock
        )
    ...
    @patch("adapter.dataset_service.DatasetFactory.saved")
    def test_copy_saved_non_empty_dataset(
        self, mock_datasetfactory_saved, mocked_dataset_service
    ):
    ...
    @patch("adapter.dataset_service.DatasetFactory.tmp")
    def test_create_dataset_tmp_with_persistence_error(
        self, mock_datasetfactory_tmp, mocked_dataset_service
    ):
    ...
    def test_delete_dataset_with_releated_test_content_persistence_error(
        self, mocked_dataset_service
    ):

        mocked_dataset_service["test_repo"].get_tests_from_dataset.return_value = [
            MagicMock(),
            MagicMock(),
        ]
    ...
    def test_delete_dataset_with_related_test_persistence_error(
        self, mocked_dataset_service
    ):
    ...
    @patch("adapter.dataset_service.DatasetFactory.saved")
    def test_update_dataset_with_persistence_error(
        self, mock_datasetfactory_saved, mocked_dataset_service
    ):
    ...
    def test_update_dataset(self, mock_datasetfactory_saved, mocked_dataset_service):
        ...
        mocked_dataset_service["dataset_repo"].update_dataset.return_value = (
            mock_updated_dataset
        )
    ...
    def test_get_all_datasets(self, mocked_dataset_service):
        ...
        mocked_dataset_service["dataset_repo"].get_all_datasets.return_value = (
            mock_datasets_list
        )
    ...
    def test_get_dataset_by_id(self, mocked_dataset_service):
        ...
        mocked_dataset_service["dataset_repo"].get_dataset_by_id.return_value = (
            mock_dataset
        )
    ...
    def test_save_tmp_with_non_tmp_dataset(self, mocked_dataset_service):
        ...
        mocked_dataset_service[
            "dataset_repo"
        ].get_dataset_by_id.return_value.is_tmp.return_value = False
    ...
    def test_save_working_copy_with_non_existent_working_copy_dataset(
        self, mocked_dataset_service
    ):
    ...
    def test_save_working_copy_with_non_working_copy_dataset(
        self, mocked_dataset_service
    ):

        mocked_dataset_service[
            "dataset_repo"
        ].get_dataset_by_id.return_value.is_working_copy.return_value = False

        with pytest.raises(InvalidDatasetOperationException):
            mocked_dataset_service["dataset_service"].save_working_copy(uuid4())

    @patch("adapter.dataset_service.DatasetFactory.saved")
    def test_save_working_copy_with_persistence_error(
        self, mock_datasetfactory_saved, mocked_dataset_service
    ):
    ...
    def test_create_working_copy_with_non_existent_working_copy(
        self, mocked_dataset_service
    ):
    ...
    def test_create_working_copy_with_persistence_error_while_creation(
        self, mock_datasetfactory_working_copy, mocked_dataset_service
    ):
    ...
    @patch("adapter.dataset_service.DatasetFactory.working_copy")
    def test_create_working_copy_with_persistence_error_while_content_copy(
        self, mock_datasetfactory_working_copy, mocked_dataset_service
    ):
